...
export const promptToSelectAssets = (
  ...
  CustomElement.selectAssets(params)
    .then(
      onNonNull((assets) =>
        CustomElement.getAssetDetails(assets.map((a) => a.id)),
      ),
    )
...
const zip = <V1, V2>(
  arr1: ReadonlyArray<V1>,
  arr2: ReadonlyArray<V2>,
): ReadonlyArray<[V1, V2]> =>
  arr1
